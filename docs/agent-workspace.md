...
Clear Recent history was additionally exercised through the deployed confirmation with pointer clicks and Enter, including a failed deletion, retry, failed replacement-session creation, and recovery that does not repeat deletion. Requests were intercepted so live history was preserved. Saved folders remained visible, and the confirmation remained clickable on desktop and at 320px. Mobile panel wrappers use an explicit dialog role on a neutral element; desktop panels retain the complementary landmark role.
