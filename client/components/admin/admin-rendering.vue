    ...
    <template v-else-if="saved">
      ...
      <section v-show="section === 'output'" id="rendering-panel-output" role="tabpanel" aria-labelledby="rendering-tab-output" class="rendering-output-panel">
        ...
        <div v-else-if="output" class="rendering-output-workspace">
          ...
          <v-alert v-if="renderNotice && output.page.id === renderNoticeFor" :type="renderFailed ? 'warning' : 'success'" variant="tonal" class="my-4">{{ renderNotice }}</v-alert>
...
export default {
  ...
  methods: {
    ...
    async rerender() {
      ...
      try { await renderPage(window.fetch.bind(window), id); if (this.disposed) return; this.renderNotice = 'The render worker finished. The inspector is reloading stored output.' } catch (error) { if (this.disposed) return; this.renderFailed = true; this.renderNotice = `The render request did not confirm completion: ${getErrorMessage(error)}. The server may still be processing it. Reload output before retrying.` } finally { if (!this.disposed) { this.rendering = false; this.renderReview = false; await this.inspectOutput() } }
    },
