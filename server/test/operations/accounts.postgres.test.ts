...
const admin = { id: 1, authVersion: 0 } as never, operator = { id: 4, authVersion: 0 } as never
suite('PostgreSQL account administration', () => {
  let previousWiki: unknown, db: Knex, store: ReturnType<typeof createAccountAdministrationStore>, enforce = false
  const account = (id: number, values: Record<string, unknown> = {}) => ({ id, name: `Account ${id}`, email: `account${id}@example.invalid`, providerKey: 'local', isActive: true, isSystem: id <= 2, isVerified: true, tfaIsActive: false, tfaSecret: null, mustChangePwd: false, location: 'Old location', jobTitle: 'Editor', timezone: 'UTC', createdAt: '2026-09-01T00:00:00Z', updatedAt: '2026-09-01T00:00:00Z', ...values })
  beforeAll(async () => {
  ...
  beforeEach(async () => {
    ...
    await db('authentication').insert([{ key: 'local', strategyKey: 'local', displayName: 'Email & password', isEnabled: true, order: 0, config: JSON.stringify({ password: 'PROVIDER-SECRET' }) }, { key: 'oidc', strategyKey: 'oidc', displayName: 'Work identity', isEnabled: true, order: 1, config: JSON.stringify({ secret: 'OIDC-SECRET' }) }])
  ...
  it('returns a bounded directory with literal search, filters and no credentials', async () => {
    ...
    expect(list.total).toBe(1); expect(list.items.map(row => row.id)).toEqual([3]); expect(list.counts.accounts).toBe(6)
    expect(JSON.stringify(list)).not.toContain('SECRET'); expect(JSON.stringify(await store.inspect(admin, 3))).not.toContain('SECRET')
  ...
  it('guards the current privileged target even when the requested change contains no groups', async () => {
    ...
    await expect(store.updateProfile(operator, 1, { ...await review(1, operator), profile: { ...protectedUser.profile, name: 'Impersonated' } })).rejects.toMatchObject({ status: 403 })
    await expect(store.setPassword(operator, 1, { ...await review(1, operator), password: 'strong-fixture-password', mustChangePassword: true })).rejects.toMatchObject({ status: 403 })
  ...
  it('clears profile fields without ending sessions for a cosmetic edit and records only changed field names', async () => {
    ...
    const next = await store.updateProfile(operator, 3, { ...await review(3, operator), profile: { ...initial.profile, name: 'New name', location: '', jobTitle: '' } })
    expect(next.profile).toMatchObject({ name: 'New name', location: '', jobTitle: '' }); expect(next.fingerprint).not.toBe(initial.fingerprint)
    expect((await db('users').where('id', 3).first()).authVersion).toBe(0)
    expect(next.history[0]).toMatchObject({ actorId: 4, action: 'profile-updated', details: { fields: ['name', 'location', 'jobTitle'], sessionsEnded: false } })
  ...
  it('ends existing sessions on membership changes, preserves private ownership and rejects escalation', async () => {
    ...
    const initial = await store.inspect(operator, 3)
    expect(initial.privateOwnershipBlocksDeletion).toBe(true); expect(initial.capabilities.delete).toBe(false)
    await expect(store.updateProfile(operator, 3, { ...await review(3, operator), profile: { ...initial.profile, groups: [1] } })).rejects.toMatchObject({ status: 403 })
    const changed = await store.updateProfile(admin, 3, { ...await review(), profile: { ...initial.profile, groups: [] } })
    expect(changed.groups).toEqual([]); expect(changed.sessionsRevokedAt).toBeTruthy(); expect((await db('users').where('id', 3).first()).authVersion).toBe(1)
  ...
  it('protects the Guest identity without inventing a ban on ordinary membership in its group', async () => {
    ...
    expect(next.profile.groups).toEqual([2,3]); expect((await store.inspect(admin, 2)).capabilities.edit).toBe(false)
  })
  it('invalidates email verification and sessions and refuses an existing normalized identity', async () => {
    ...
    await expect(store.updateProfile(admin, 3, { ...await review(), profile: { ...initial.profile, email: 'ACCOUNT4@EXAMPLE.INVALID' } })).rejects.toMatchObject({ status: 409 })
    const changed = await store.updateProfile(admin, 3, { ...await review(), profile: { ...initial.profile, email: 'Changed@Example.Invalid' } })
    expect(changed.email).toBe('changed@example.invalid'); expect(changed.isVerified).toBe(false); expect((await db('users').where('id', 3).first()).authVersion).toBe(1)
  })
  it('accepts only one concurrent review and rejects ABA and changed access policy', async () => {
    const initial = await review(), requests = await Promise.allSettled([store.act(admin, 3, { ...initial, action: 'deactivate' }), store.act(admin, 3, { ...initial, action: 'deactivate' })])
    ...
    const current = await review(); await db('groups').where('id', 3).update({ permissions: JSON.stringify(['read:pages']) })
  ...
  it('uses actual provider capabilities and never mistakes an unverified setup secret for enrollment', async () => {
    ...
    expect(external.twoFactor).toBe('provider-managed'); expect(external.capabilities.password).toBe(false); expect(external.capabilities.actions).not.toContain('require-2fa')
    ...
    expect(required.twoFactor).toBe('enrollment-required'); expect((await db('users').where('id', 3).first()).tfaSecret).toBeNull()
    enforce = true; expect((await store.inspect(admin, 3)).capabilities.actions).not.toContain('disable-2fa')
  })
  it('resets an enrolled authenticator into required enrollment and ends sessions', async () => {
    ...
    expect(next.twoFactor).toBe('enrollment-required'); expect((await db('users').where('id', 3).first()).authVersion).toBe(1)
  ...
  it('replaces a local password with a real hash and records no credential', async () => {
    ...
    expect(await bcrypt.compare(password, persisted.password)).toBe(true); expect(persisted.authVersion).toBe(1); expect(next.mustChangePassword).toBe(true)
    expect(JSON.stringify(next)).not.toContain(password); expect(JSON.stringify(next)).not.toContain(persisted.password)
    await expect(store.setPassword(admin, 3, { ...await review(), password: '🙂'.repeat(20), mustChangePassword: true })).rejects.toMatchObject({ status: 400 })
  })
  it('rolls back account state, session revocation and history together on persistence failure', async () => {
    ...
    try { await expect(store.act(admin, 3, { ...await review(), action: 'deactivate' })).rejects.toThrow() } finally { await db.raw('ALTER TABLE "userAdministrationEvents" DROP CONSTRAINT account_test_failure') }
    expect((await db('users').where('id', 3).first())).toMatchObject({ isActive: true, authVersion: 0, adminRevision: '' }); expect(await db('userAdministrationEvents')).toHaveLength(0)
  })
  ...
  it('creates an account with reviewed membership and a password hash, with no implicit mail', async () => {
    ...
    const created = await store.create(operator, { fingerprint: options.fingerprint, profile, providerKey: 'local', isVerified: true, mustChangePassword: true, password: 'creation-password-12!', reason: 'Create a test author' })
    ...
    await expect(store.create(operator, { fingerprint: options.fingerprint, profile: { ...profile, groups: [1], email: 'escalation@example.invalid' }, providerKey: 'oidc', isVerified: true, mustChangePassword: false, reason: 'Attempt group escalation' })).rejects.toMatchObject({ status: 403 })
  })
  it('serializes competing normalized account creates and rejects a stale creation policy', async () => {
    const options = await store.creationOptions(admin), input = { fingerprint: options.fingerprint, profile: { name: 'Work account', email: 'work@example.invalid', location: '', jobTitle: '', timezone: 'UTC', groups: [3] }, providerKey: 'oidc', isVerified: true, mustChangePassword: false, reason: 'Provision a work account' }
    const result = await Promise.allSettled([store.create(admin, input), store.create({ id: 6, authVersion: 0 } as never, { ...input, fingerprint: (await store.creationOptions({ id: 6, authVersion: 0 } as never)).fingerprint })])
  ...
  it('preserves private ownership/history and reassigns public contributions only after reviewed deletion', async () => {
    ...
    await db('comments').insert({ id: 1, authorId: 3 }); await db('userKeys').insert({ userId: 3, kind: 'resetPwd', token: 'synthetic' })
    ...
    expect(await db('users').where('id', 3).first()).toBeUndefined(); expect(await db('userKeys').where('userId', 3)).toHaveLength(0)
    expect(await db('pages').first()).toMatchObject({ ownerId: null, creatorId: 5, authorId: 5, visibility: 'public' }); expect((await db('comments').first()).authorId).toBe(5)
  ...
  it('persists welcome-email acceptance or failure without implying delivery or storing its body', async () => {
    ...
    current = await store.inspect(admin, 3); expect(current.history[0]?.action).toBe('welcome-email-accepted'); expect(JSON.stringify(current.history)).not.toContain(prepared.email)
    const failed = await store.prepareWelcome(admin, 3, { ...await review(), reason: 'Retry an explicit welcome' }); await store.finishWelcome(3, failed.requestId, false)
  ...
  it('invalidates pending recovery and continuation keys and fences issuance against old login proof', async () => {
    globalThis.WIKI = { Error: { AuthValidationTokenInvalid: class extends Error {} }, models: { knex: db, users: { query: (tx = db) => ({ findById: (id: number) => tx('users').where('id', id).first() }) }, userKeys: { query: (tx = db) => ({ findOne: (input: Record<string, unknown>) => tx('userKeys').where(input).first(), insert: (input: Record<string, unknown>) => tx('userKeys').insert(input), deleteById: (id: number) => tx('userKeys').where('id', id).delete() }) } } } as never
    ...
    const competing = await Promise.allSettled([UserKey.validateToken({ kind: 'resetPwd', token: next }), UserKey.validateToken({ kind: 'resetPwd', token: next })])
  ...
  it('advances persisted session authority for legacy group membership and permission changes', async () => {
    ...
    globalThis.WIKI = { auth: { checkExclusiveAccess: (user: unknown, include: string[], exclude: string[]) => include.some(p => permissionList(user).includes(p)) && !exclude.some(p => permissionList(user).includes(p)), reloadGroups: async () => {}, revokeUserTokens: () => {} }, events: { outbound: { emit: () => {} } }, models: { knex: db, groups: { query: (tx = db) => ({ patch: (input: Record<string, unknown>) => tx('groups').update({ ...input, permissions: JSON.stringify(input.permissions), pageRules: JSON.stringify(input.pageRules) }), deleteById: (id: number) => tx('groups').where('id', id).delete() }) } } } as never
